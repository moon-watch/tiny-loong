...
//EUEN
    ...
    assign csr_euen = {31'b0, csr_euen_fpe};
//ECFG
    always @(posedge clk) begin
        if (rst)
            csr_ecfg_lie <= 13'h0;
    ...
    assign csr_ecfg = {19'b0, csr_ecfg_lie[12:11], 1'b0, csr_ecfg_lie[9:0]};
    assign csr_ecfg_lie_r = csr_ecfg[12:0];
//ESTAT
    //IS
...
//BADV
    //Vaddr
    assign wb_ex_addr_err   =  wb_ecode == `ECODE_TLBR
                            || wb_ecode == `ECODE_ADE
                            || wb_ecode == `ECODE_ALE
                            || wb_ecode == `ECODE_PIL
                            || wb_ecode == `ECODE_PIS
...
//TLBEHI
    assign wb_ex_pg_err =  wb_ecode == `ECODE_TLBR
                        || wb_ecode == `ECODE_PIL
                        || wb_ecode == `ECODE_PIS
                        || wb_ecode == `ECODE_PIF
                        || wb_ecode == `ECODE_PME
...
//PGD
    assign csr_pgd = {(csr_badv[31] ? csr_pgdh_base : csr_pgdl_base), 12'b0};
//TLBRENTRY
    always @(posedge clk) begin
